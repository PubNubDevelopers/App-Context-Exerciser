    ...
    subscriptionSet.addListener({
        objects: async objectEvent => {
            ...
            if (objectEvent.message.type === 'uuid' && 
                ...
                else if (objectEvent.message.type === 'channel' && 
                objectEvent.message.event === 'set')
                {
                ...
                else if (objectEvent.message.type === 'membership' &&
                objectEvent.message.event === 'set')
                {
                    //  A new user has been added to a channel we are a member of
                    createMembership(objectEvent.message.data.uuid.id, objectEvent.message.data.channel.id)
                ...
                objectEvent.message.event === 'delete')
                {
                    ...
                    deleteMembership(objectEvent.message.data.uuid.id, objectEvent.message.data.channel.id)
                }
        }
    })
}
        ...
        if (users.data.length > 0)
        {
            for (var i = 0; i < users.data.length; i++)
            {
                ...
                for (var j = 0; j < userMemberships.data.length; j++)
                {
                    ...
                    {
                        createMembership(users.data[i].id, userMemberships.data[j].channel.id)
                    }
                }
            }
        }
...
function addChannelToUI(channelData)
{
    ...
    if (channelName === null) {channelName = "Not Specified"} //  text deleted
    var channelDescription = channelData.description
    if (channelDescription === null) {channelDescription = "Not Specified"}
    var channelType = channelData.type
    if (channelType === null) {channelType = "Not Specified"}
    var channelStatus = channelData.status
    if (channelStatus === null) {channelStatus = "Not Specified"}
    ...
    if (channelElement)
    {
        ...
        if (channelCustom)
        {
            ...
            var channelCustomDiv = document.getElementById(channelId + "-customFields")
            if (channelCustomDiv)
            {
        ...
        else
        {
            ...
            {
                var channelDescriptionDiv = document.getElementById(channelId + "-channelDescription")
            ...
            if (channelStatus)
            {
                var channelStatusDiv = document.getElementById(channelId + "-channelStatus")
    ...
    {
        if (!channelName) {channelName = "Not Specified"}
        if (!channelDescription) {channelDescription = "Not Specified"}
        if (!channelType) {channelType = "Not Specified"}
        if (!channelStatus) {channelStatus = "Not Specified"}
        var customFields = ""
        if (!channelCustom) 
        { 
...
function addUserToUI(userData)
{
    ...
    if (userName === null) {userName = "Not Specified"}
    var userEmail = userData.email
    if (userEmail === null) {userEmail = "Not Specified"}
    var userProfileUrl = userData.profileUrl
    if (userProfileUrl === null) {userProfileUrl = "Not Specified"}
    var userExternalId = userData.externalId
    if (userExternalId === null) {userExternalId = "Not Specified"}
    var userType = userData.type
    if (userType === null) {userType = "Not Specified"}
    var userStatus = userData.status
    if (userStatus === null) {userStatus = "Not Specified"}
    ...
    if (channelElement)
    {
        ...
        else
        {
            ...
            if (userProfileUrl)
            {
                var userProfileUrlDiv = document.getElementById(userId + "-userProfileUrl")
                userProfileUrlDiv.innerHTML = userProfileUrl
            }
            if (userExternalId)
            {
                var userExternalIdDiv = document.getElementById(userId + "-userExternalId")
    ...
    else
    {
        ...
        if (!userEmail) {userEmail = "Not Specified"}
        if (!userProfileUrl) {userProfileUrl = "Not Specified"}
        if (!userExternalId) {userExternalId = "Not Specified"}
        if (!userType) {userType = "Not Specified"}
        if (!userStatus) {userStatus = "Not Specified"}
        var customFields = ""
        if (!userCustom) 
        { 
            customFields = `<div class="contextObjectRow">No Custom Fields Specified</div>`
...
function generateCustomDataRow(userCustom)
{
    ...
    for (var i = 0; i < Object.keys(userCustom).length; i++)
    {
        var customDataRow = `
        <div class="contextObjectRow">
            <div class="contextHeading">${Object.keys(userCustom)[i]}</div><div class="contextData">${userCustom[Object.keys(userCustom)[i]]}</div>
        </div>
        `
...
{
    var membershipElement = document.getElementById(userId + "-" + channelId + "-membership")
